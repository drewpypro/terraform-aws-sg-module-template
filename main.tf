variable "security_group_files" {
  default = fileset("${path.module}/security_groups", "*.json")
}

# Loop through JSON files to create security groups
resource "aws_security_group" "security_groups" {
  for_each = { for file in var.security_group_files : file => jsondecode(file("${path.module}/security_groups/${file}")) }
...
resource "aws_security_group_rule" "rules" {
  for_each = flatten([
    for sg_file in var.security_group_files : [
